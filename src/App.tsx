...
function App() {
  ...
  const [lastCleaned, setLastCleaned] = useState(
    load("lastCleaned") || getNow()
  );
  const [lastHealthy, setLastHealthy] = useState(
    load("lastHealthy") || getNow()
  );
  ...
  const setCurrentMood = useCallback(() => {
    ...
    if (Hunger.needsFulfilment(lastFed)) {
      ...
      if (
        Loneliness.needsFulfilment(lastPetted) &&
        Dirtiness.needsFulfilment(lastFed)
      )
        setMood(moods.hungryLonelyAndDirty);
      else if (Loneliness.needsFulfilment(lastPetted))
        setMood(moods.hungryAndLonely);
      else if (Dirtiness.needsFulfilment(lastCleaned))
        setMood(moods.hungryAndDirty);
    ...
    getIsSick(lastFed, lastPetted, lastCleaned)
      ? setMood(moods.sick)
      : setLastHealthy(getNow());
    sleepiness > intervals.sleepiness * 2
      ? setMood(moods.sick)
      : setLastHealthy(getNow());
    if (getIsDead(lastHealthy) || Math.round(age / 86400) > 365)
      setMood(moods.dead);
